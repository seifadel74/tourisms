...
import { fetchHotels, updateHotel, deleteHotel } from '../api/hotel';
import { fetchYachts, updateYacht, deleteYacht } from '../api/yacht';
...
const AdminDashboard: React.FC = () => {
  ...
  const addYacht = (yacht: Yacht) => {
    setYachts(prev => [...prev, { ...yacht, id: Date.now() }]);
  };
  return (
  ...
  async function handleDelete(id: number) {
    if (window.confirm('هل أنت متأكد من حذف الفندق؟')) {
      try {
        ...
        toast.success(t('admin.hotelDeletedSuccess'));
      } catch (error) {
        toast.error(t('admin.hotelDeleteFailed'));
      }
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editHotel) {
        ...
        toast.success(t('admin.hotelUpdatedSuccess'));
      } else {
        // Create new hotel with mock data for now
        const newHotel = { 
          ...hotelData, 
          id: Date.now(),
          name: form.name,
        ...
        toast.success(t('admin.hotelAddedSuccess'));
      }
      setModalOpen(false);
    } catch (error) {
  ...
  return (
    <div className="hotels-admin">
      <div className="header">
        <h2>الفنادق</h2>
        <button className="home-buttons-btn" onClick={openAdd}>{t('admin.addNewHotel')}</button>
      </div>
      ...
      <AdminFormModal
        isOpen={modalOpen}
        title={editHotel ? t('admin.editHotel') : t('admin.addNewHotel')}
        onSubmit={handleSubmit}
        onClose={() => setModalOpen(false)}
        submitButtonText={editHotel ? t('admin.saveChanges') : t('admin.add')}
      >
        <div style={{ display: 'grid', gap: '16px', maxHeight: '70vh', overflowY: 'auto' }}>
          ...
          />
          <textarea 
            placeholder={t('admin.hotelDescription')} 
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editYacht) {
        ...
        // Create new yacht with mock data for now
        const newYacht = { 
          ...yachtData, 
          id: Date.now(),
          description: form.description,
